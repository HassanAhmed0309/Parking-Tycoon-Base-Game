...
namespace ArcadeBridge.ArcadeIdleEngine.Gathering
{
	...
	public class GatherableDefinition : ScriptableObject
	{
		...
		public PickablePool GatheringOutputPickablePool { get; private set; }
		...
		public TweenFeedback PickableSpawnedFeedback  { get; private set; }
		...
		/// <summary>
		/// Takes current hit point and damage amount, then calculates whether it produces new pickables as a reward
		/// </summary>
		/// <returns></returns>
		public bool Gather(int currentHitPoint, int newHitPoint, out GatherableReward gatherableReward)
		{
			...
			gatherableReward.PickablePool = GatheringOutputPickablePool;
	...
	{
		public PickablePool PickablePool;
		public int Amount;
	}
	...
	public class HitPointThreshold
	{
		...
		public void Initialize()
		{
			...
			for (int i = 0; i < _value.Length; i++)
			{
				...
				_calculatedValue[i].PickableSpawnCount = threshold.PickableSpawnCount;
			}
		}
		...
		/// </summary>
		/// <returns>Pickable count that needs to be spawn</returns>
		public int GetOutputCount(int from, int to)
		{
			...
			{
				totalReward += _calculatedValue[firstIndex + i].PickableSpawnCount;
			}
			return totalReward;
		}
